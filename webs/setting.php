<?php
/**
 * motor_drive_room_settings.php - KBS Monitoring Settings (Updated with Sensor LEDs)
 */
...
<body class="app-container">
    ...
    <script>
        ...
        client.on('connect', () => {
            ...
            // Subscribe sensor topics เพื่อเช็คสถานะ Online
        ...
        // Watchdog เช็ค Sensor Offline ทุก 5 วินาที
        setInterval(() => {
            const now = Date.now();
            for (let i = 1; i <= 5; i++) {
                if (now - sensorLastSeen[i] > 15000) { // เกิน 15 วินาทีถือว่า Offline
        ...
        function publishLight(state) { 
            client.publish("kbs/driveroom1/light1", state ? "Active" : "Unactive", { qos: 1, retain: true }); 
        }
    </script>
</body>
</html>