...
// 1. AUTO FETCH LAST DATE & LAST UPLOAD TIME
// ดึงทั้งวันที่รายงาน (report_date) และวันเวลาที่บันทึกข้อมูลเข้า SQL (created_at)
$date_query = $mysqli->query("SELECT MAX(report_date) as last_date, MAX(created_at) as last_upload FROM vehicle_utilization");
$date_row = $date_query->fetch_assoc();
$f_date = $date_row['last_date'] ?? date('Y-m-d');
$last_upload = $date_row['last_upload'] ?? null; 
...
<body class="p-6" onclick="handleBodyClick(event)">
    ...
    <main class="max-w-[1600px] mx-auto">
        ...
        <header class="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 no-pdf header-container">
            <div>
                ...
                <div class="mt-2 flex flex-col gap-1">
                    <p class="text-slate-400 font-bold uppercase text-[10px] tracking-[0.2em] leading-none">
                        อัปโหลดล่าสุด: <span class="text-sky-600"><?= $last_upload ? date('d M Y | H:i:s', strtotime($last_upload)) : 'ไม่พบข้อมูล' ?></span>
                    </p>
                </div>
            </div>
            <div class="flex items-center gap-3 bg-white p-2 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <button onclick="window.print()" class="bg-slate-900 text-white w-10 h-10 rounded-xl flex items-center justify-center hover:bg-sky-600 transition-colors shadow-sm"><i class="fas fa-print"></i></button>
            </div>
        </header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 mb-10 text-center">
            <div class="lg:col-span-3">
                <div class="flat-card p-6 h-full flex flex-col items-center justify-center relative min-h-[350px]">
                    ...
                    <div class="mt-6 w-full text-[12px] font-black italic space-y-2">
                        <div class="flex justify-between text-green-800"><span>พร้อมใช้งาน:</span><span><?= $p_ready_y ?>%</span></div>
                        <div class="flex justify-between text-rose-400"><span>ไม่พร้อมใช้งาน:</span><span><?= $p_ready_n ?>%</span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2">
                <div class="flat-card p-6 h-full flex flex-col items-center justify-center relative min-h-[350px]">
                    <p class="text-[18px] font-black text-emerald-600 uppercase absolute top-8 italic">พร้อมใช้งาน</p>
                    ...
                    <div class="mt-6 w-full text-[10px] font-black italic space-y-2 uppercase">
                        <div class="flex justify-between text-sky-500"><span>กำลังใช้งาน:</span><span><?= number_format($totals['used']) ?> (<?= $p_used ?>%)</span></div>
                        <div class="flex justify-between text-indigo-500"><span>รอมอบหมาย:</span><span><?= number_format($totals['assigned']) ?> (<?= $p_asgn ?>%)</span></div>
            ...
            <div class="lg:col-span-2">
                <div class="flat-card p-6 h-full flex flex-col items-center justify-center relative min-h-[350px]">
                    <p class="text-[18px] font-black text-rose-600 uppercase absolute top-8 italic">ไม่พร้อมใช้งาน</p>
                    <div class="relative w-28 h-28">
            ...
            <div class="lg:col-span-5 grid grid-rows-2 gap-3 text-center font-black italic">
                <div class="grid grid-cols-3 gap-3">
                    <div class="flat-card border-t-4 border-t-green-800 p-4 flex flex-col justify-center items-center">
                        <span class="text-[12px] text-green-800 uppercase leading-none font-black">พร้อมใช้งาน(คัน)</span>
                        <div class="text-2xl text-green-900 mt-1 font-black italic"><?= number_format($totals['ready_y']) ?></div>
                    </div>
                    <div class="flat-card border-t-4 border-t-rose-400 p-4 flex flex-col justify-center items-center">
                        <span class="text-[12px] text-rose-400 uppercase leading-none">ไม่พร้อมใช้งาน(คัน)</span>
                    ...
                    <div class="flat-card border-t-4 border-t-indigo-500 p-4 flex flex-col justify-center items-center">
                        <span class="text-[12px] text-indigo-500 uppercase leading-none">รอมอบหมาย(คัน)</span>
                ...
                <div class="grid grid-cols-3 gap-3">
                    <div class="flat-card border-t-4 border-t-sky-500 p-4 flex flex-col justify-center items-center">
                        <span class="text-[12px] text-sky-500 uppercase leading-none">ถูกใช้งาน(คัน)</span>
                    ...
                    <div class="flat-card border-t-4 border-t-amber-500 p-4 flex flex-col justify-center items-center">
                        <span class="text-[12px] text-amber-500 uppercase leading-none">รอใช้งาน(คัน)</span>
                    ...
                    <div class="flat-card border-t-4 border-t-rose-700 p-4 flex flex-col justify-center items-center">
                        <span class="text-[12px] text-rose-700 uppercase leading-none">เสีย(คัน)</span>
            ...
            <?php foreach ($rows as $r): $style = getVehicleStyle($r['vehicle_type']); ?>
                <div class="flat-card p-5 flex flex-col items-center text-center group transition-colors hover:border-sky-300">
                    ...
                    <div class="w-full space-y-4">
                        <div class="space-y-1">
                            <p class="text-[10px] font-black text-green-800 uppercase italic leading-none">พร้อมใช้งาน</p>
                            <div class="grid grid-cols-3 gap-1 font-black italic">
                                <div class="bg-sky-100 p-2 rounded-lg text-sky-800 text-[13px]"><span class="block text-[8px] uppercase">กำลังใช้งาน</span><?= (int)$r['in_use'] ?></div>
                                <div class="bg-indigo-100 p-2 rounded-lg text-indigo-800 text-[13px]"><span class="block text-[8px] uppercase">รอมอบหมาย</span><?= (int)($r['assign_reuire'] ?? 0) ?></div>
                                <div class="bg-amber-100 p-2 rounded-lg text-amber-800 text-[13px]"><span class="block text-[8px] uppercase">รอใช้งาน</span><?= (int)$r['standby'] ?></div>
                            </div>
                        </div>
                        <div class="space-y-1 border-t pt-2 border-slate-100">
                            <p class="text-[10px] font-black text-rose-700 uppercase italic leading-none">ไม่พร้อมใช้งาน</p>
                            <div class="grid grid-cols-2 gap-1 font-black italic">
                                <div class="bg-orange-100 p-2 rounded-lg text-orange-800 text-[13px]"><span class="block text-[8px] uppercase">ซ่อมบำรุง</span><?= (int)$r['pm_plan'] ?></div>
                                <div class="bg-rose-100 p-2 rounded-lg text-rose-800 text-[13px]"><span class="block text-[8px] uppercase">เสีย</span><?= (int)$r['breakdown'] ?></div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        // Render Charts
        new Chart(document.getElementById('healthDonut'), { type: 'doughnut', data: { labels: ['Ready (Y)', 'Ready (N)'], datasets: [{ data: [<?= $totals['ready_y'] ?>, <?= $totals['ready_n'] ?>], backgroundColor: ['#026242', '#fb7185'], borderWidth: 0, cutout: '80%' }] }, options: cfg });
        new Chart(document.getElementById('readyBreakdownDonut'), { type: 'doughnut', data: { labels: ['Used', 'Assigned', 'Standby', 'Not Ready'], datasets: [{ data: [<?= $totals['used'] ?>, <?= $totals['assigned'] ?>, <?= $totals['stb'] ?>, <?= $totals['ready_n'] ?>], backgroundColor: ['#0ea5e9', '#6366f1', '#f59e0b', '#f1f5f9'], borderWidth: 0, cutout: '80%' }] }, options: cfg });
        new Chart(document.getElementById('maintenanceDonut'), { type: 'doughnut', data: { labels: ['PM Plan', 'Breakdown'], datasets: [{ data: [<?= $totals['pm'] ?>, <?= $totals['brk'] ?>], backgroundColor: ['#f59e0b', '#be123c'], borderWidth: 0, cutout: '80%' }] }, options: cfg });

        // --- AUTO REFRESH LOGIC ---
        let currentLastUpload = "<?= $last_upload ?>";
        function checkDataUpdate() {
            // เรียกไฟล์ตัวเองแต่ส่ง param พิเศษไปเพื่อดึงแค่เวลา (หรือสร้างไฟล์ใหม่ตามคำแนะนำก่อนหน้า)
            fetch('check_update.php') 
                .then(response => response.text())
                .then(latestUpload => {
                    if (latestUpload.trim() !== "" && latestUpload.trim() !== currentLastUpload) {
                        location.reload(); 
                    }
                })
                .catch(err => console.log("Update check paused."));
        }
        setInterval(checkDataUpdate, 30000); // เช็คทุก 30 วินาที
